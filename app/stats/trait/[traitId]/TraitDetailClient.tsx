...
export default function TraitDetailPage({ params }: PageProps) {
  ...
  return <TraitDetailView traitId={traitId} />;
}

/**
 * The page itself, entered only with an id of the right shape.
 *
 * Separate from the export so that check can happen before any state exists. Held in one
 * component, the rejection would sit above every hook in the file and change how many of
 * them run between one render and the next.
 */
function TraitDetailView({ traitId }: { traitId: string }) {
