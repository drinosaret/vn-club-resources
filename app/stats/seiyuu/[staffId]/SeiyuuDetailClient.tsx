...
export default function SeiyuuDetailPage({ params }: PageProps) {
  ...
  const initialTab: TabId = tabFromUrl && VALID_TABS.includes(tabFromUrl) ? tabFromUrl : 'summary';
  const initialPage = pageFromUrl ? Math.max(1, parseInt(pageFromUrl, 10) || 1) : 1;

  const [stats, setStats] = useState<SeiyuuStatsData | null>(null);
  const [vns, setVns] = useState<TagVN[]>([]);
  const [vnsPage, setVnsPage] = useState(initialTab === 'novels' ? initialPage : 1);
  const [vnsTotal, setVnsTotal] = useState(0);
  const [vnsPages, setVnsPages] = useState(1);
  const [characters, setCharacters] = useState<SeiyuuVoicedCharacter[]>([]);
  ...
  useEffect(() => {
    if (characters.length > 0) previousCharsRef.current = characters;
  }, [characters]);
  ...
  useEffect(() => {
    ...
    if (tabChanged) {
      setActiveTab(urlTab);
      if (urlTab === 'novels') setVnsPage(urlPage);
      else if (urlTab === 'characters') setCharsPage(urlPage);
      setVns([]);
      setCharacters([]);
    } else if (urlTab === 'novels' && urlPage !== vnsPage && vns.length > 0) {
      setVnsPage(urlPage);
      loadNovels(urlPage);
  ...
  }, [activeTab, vns.length, characters.length]);
  ...
  const loadInitialData = async (forceRefresh = false) => {
    ...
    try {
      const statsData = await vndbStatsApi.getSeiyuuStats(staffId, { nocache: forceRefresh });

      if (!statsData) {
        setError('Voice actor not found or backend unavailable.');
        return;
      }

      setStats(statsData);
    } catch {
      setError('Failed to load voice actor data.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const loadNovels = async (page = 1, spoilerLevel: number = spoilerFilter, olang: string = languageFilter) => {
    ...
    if (vnsPrefetchCacheRef.current.has(cacheKey)) {
      ...
      if (cached.page < cached.pages) {
        ...
        if (!vnsPrefetchCacheRef.current.has(nextKey)) {
          vndbStatsApi.getSeiyuuVNsWithTags(staffId, cached.page + 1, 24, 'rating', spoilerLevel, olang === 'all' ? undefined : olang)
            .then(result => {
              if (result) {
    ...
    try {
      const response = await vndbStatsApi.getSeiyuuVNsWithTags(staffId, page, 24, 'rating', spoilerLevel, olang === 'all' ? undefined : olang);
      if (response) {
        setVns(response.vns);
        ...
        if (response.page < response.pages) {
          ...
          if (!vnsPrefetchCacheRef.current.has(nextKey)) {
            vndbStatsApi.getSeiyuuVNsWithTags(staffId, response.page + 1, 24, 'rating', spoilerLevel, olang === 'all' ? undefined : olang)
              .then(result => {
                if (result) {
                  vnsPrefetchCacheRef.current.set(nextKey, result);
                  preloadVNImages(result.vns);
  ...
  const loadCharacters = async (page = 1) => {
    ...
    if (charsPrefetchCacheRef.current.has(cacheKey)) {
      ...
      // Prefetch next page
      if (cached.page < cached.pages) {
        const nextKey = `chars-${cached.page + 1}`;
        if (!charsPrefetchCacheRef.current.has(nextKey)) {
          vndbStatsApi.getSeiyuuCharacters(staffId, cached.page + 1, 24, 'name')
            .then(result => {
              if (result) {
    ...
    setIsLoadingTab(true);
    try {
      const response = await vndbStatsApi.getSeiyuuCharacters(staffId, page, 24, 'name');
      if (response) {
        setCharacters(response.characters);
        previousCharsRef.current = response.characters;
        ...
        // Prefetch next page
        if (response.page < response.pages) {
          const nextKey = `chars-${response.page + 1}`;
          if (!charsPrefetchCacheRef.current.has(nextKey)) {
            vndbStatsApi.getSeiyuuCharacters(staffId, response.page + 1, 24, 'name')
              .then(result => {
                if (result) {
                  charsPrefetchCacheRef.current.set(nextKey, result);
                  preloadCharacterImages(result.characters);
  ...
  const handleCharsPageChange = useCallback((newPage: number) => {
    if (newPage >= 1 && newPage <= charsPages) {
      loadCharacters(newPage);
      updateUrl(activeTab, newPage);
    }
  }, [activeTab, charsPages, updateUrl]);
  ...
  const staffDisplayName = getEntityDisplayName(staff, preference);
  const staffAltName = staffDisplayName === staff.name ? staff.original : staff.name;
                ...
                <div className={`transition-opacity duration-150 ${isLoadingTab ? 'opacity-60 pointer-events-none' : ''}`}>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                    {displayChars.map((char) => (
                      <VoicedCharacterCard key={char.id} character={char} />
                    ))}
                  </div>
                  {charsPages > 1 && (
...
function VoicedCharacterCard({ character: char }: { character: SeiyuuVoicedCharacter }) {
  ...
  const maxVNs = 2;
  const shownVNs = char.vns.slice(0, maxVNs);
  const overflowCount = char.vns.length - maxVNs;
      ...
      <div className="flex-1 min-w-0">
        ...
        {shownVNs.length > 0 && (
          <div className="flex flex-wrap gap-1 mt-1.5">
            {shownVNs.map((vn) => {
              const vnTitle = getDisplayTitle({ title: vn.title, title_jp: vn.title_jp, title_romaji: vn.title_romaji }, preference);
              return (
                <span
                  key={vn.id}
