---
description: Learn Japanese with visual novels using JL - a popup dictionary with Anki mining, pitch accents, and frequency display. Step-by-step setup guide for beginners.
sitemap:
  priority: 0.8
  changefreq: monthly
faq:
  - q: "What is JL and how does it help with reading visual novels?"
    a: "JL is a free popup dictionary for Windows that displays definitions, readings, pitch accents, and word frequencies when you hover over Japanese text. It connects to Textractor via WebSocket or clipboard to receive text from visual novels, and integrates with Anki for one-click vocabulary mining."
  - q: "How do I connect JL to Textractor?"
    a: "In JL's preferences under 'Main Window', enable 'Enable WebSocket text capture'. The default address ws://127.0.0.1:6677 works with the Textractor WebSocket extension. Alternatively, JL can receive text via clipboard capture, which works automatically if Textractor copies text to your clipboard."
  - q: "What dictionaries should I use with JL?"
    a: "JL comes with JMdict, Kanjidic, and JMnedict built in. For additional dictionaries, download from Marv's Yomitan Dictionaries collection on GitHub. JL supports both Yomitan and Nazeka format dictionaries including monolingual Japanese dictionaries like 大辞林."
  - q: "Can JL work with OCR for games that can't be text hooked?"
    a: "Yes. Using Tsukikage, a companion app for JL, you can connect JL to OwOCR for games that don't support text hooking. Tsukikage reads JSON output from OwOCR and sends text to JL only when you hover over the designated screen region."
---


# JL Setup Guide: Pop-Up Dictionary for Visual Novels
...
## Tsukikage (OwOCR Integration)

[Tsukikage](https://github.com/rampaa/Tsukikage) is a companion application to JL that allows JL to connect with OwOCR seamlessly. Instead of sending the raw OCR text to JL, Tsukikage will read JSON output from OwOCR (includes text coordinates) and sends that to JL only when you hover over the designated region on your screen.

> **Note:** This requires that you have already installed and set up OwOCR. If you haven't yet, please see the [OwOCR Guide](/owocr-guide).

![Tsukikage in action](/assets/tsukikage.webp)

### Requirements

- **[Tsukikage](https://github.com/rampaa/Tsukikage/releases)** (download the latest release)
- **[.NET Desktop Runtime 10.0+](https://dotnet.microsoft.com/en-us/download/dotnet/10.0)** (install if you don't already have it)
- **[OwOCR](https://github.com/AuroraWright/owocr)** configured for JSON output via WebSocket
- **JL v4.0.0+** (has a built-in Tsukikage profile)

### OwOCR Configuration

OwOCR needs to output JSON with text coordinates over WebSocket for Tsukikage to work. In the OwOCR config (tray icon > Configure, or run `owocr_config`):

Under **General**:
- **read_from:** `screencapture`
- **write_to:** `websocket`
- **websocket_port:** must match Tsukikage's `OcrJsonInputWebSocketAddress` port (both default to `7331`)
- **output_format:** `json`

Under **Engines**:
- **engine:** `oneocr` on Windows, `alivetext` on macOS, `meikiocr` on Linux (you can also use `Google Lens` or another online engine if you prefer)
- Only enable the engine(s) you actually use, as extra engines significantly increase startup time

Under **Screen capture**:
- **screen_capture_area:** `window` is strongly recommended for accuracy
- **screen_capture_delay_seconds:** if CPU usage is too high, increase this value (1.5-3 seconds may work well for OneOCR, but this will vary depending on your PC specs). Set to `-1` to disable automatic capture and use the `screen_capture_combo` hotkey instead

Under **Processing**:
- **join_lines:** enabled
- **join_paragraphs:** disabled
- **paragraph_separator:** `\n\n` recommended
- **reorder_text:** enabled

> **Important:** You must start OwOCR **after** the window you want to OCR is already open.

### JL Configuration

If you are using JL v4.0.0+, there will automatically be a Tsukikage profile with everything configured automatically.

1. Right-click on the main JL window and open the preferences window
2. Under **General**, look for the **Profiles** option
3. Change it to **Tsukikage** and click **Save**

The default WebSocket address is `ws://127.0.0.1:8768`, which must match the `OutputWebSocketAddress` setting in Tsukikage.

> It is recommended that you minimize the main JL window, as you won't need to interact with it while using Tsukikage.

**Other useful settings:**

**Lookup requires Lookup Key press**: Under the General tab in preferences, enable this if you want to require a key to look up words, preventing accidental lookups.

**Enable mining mode**: Just as you would with JL, while looking up a word, you can enter "mining mode" by pressing the associated hotkey. Alternatively, you can (under the Popup tab in preferences) enable **Automatically enable mining mode** and set **Mining mode activation delay (in milliseconds)** if you want to automatically enter mining mode after a certain amount of time.

**Vertical text popup settings**: The following settings under the Popup tab in preferences all behave in the same way as their non-vertical counterparts, but only apply to vertical text received from Tsukikage:
- X Offset (Vertical Text)
- Y Offset (Vertical Text)
- Popup position relative to cursor (Vertical Text)
- Flip (Vertical Text)

### Tsukikage Configuration

All settings and their explanations can be found in the [Tsukikage.ini](https://github.com/rampaa/Tsukikage/blob/master/Tsukikage/Tsukikage.ini) file. The defaults should work out of the box with JL's Tsukikage profile.

If you are also using a texthooker, you can set **TextHookerWebSocketAddress** to its WebSocket address to help correct OCR mistakes.
