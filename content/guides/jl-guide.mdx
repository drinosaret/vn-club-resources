...
Under **Engines**:
- **engine:** `oneocr` on Windows, `alivetext` on macOS, `meikiocr` on Linux
...
- **screen_capture_delay_seconds:** if CPU usage is too high, increase this value (1.5-3 seconds works well for OneOCR). Set to `-1` to disable automatic capture and use a hotkey instead
