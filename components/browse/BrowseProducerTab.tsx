...
import { vndbStatsApi, BrowseProducerItem, BrowseProducerParams } from '@/lib/vndb-stats-api';
import { useBrowseProducers } from '@/lib/vndb-stats-cached';
...
import { EntityTable, EntityColumn, NameCell, BadgeCell, CountCell } from './EntityTable';
...
interface BrowseProducerTabProps {
  isActive?: boolean;
}

export function BrowseProducerTab({ isActive = true }: BrowseProducerTabProps) {
  const { preference } = useTitlePreference();
  const [params, setParams] = useState<BrowseProducerParams>({
    sort: 'vn_count',
    sort_order: 'desc',
    page: 1,
  ...
  const { data, error, isLoading, isValidating } = useBrowseProducers(debouncedParams, isActive);
  ...
  const updateParams = (updates: Partial<BrowseProducerParams>) => {
  ...
  const handlePrefetchPage = useCallback((targetPage: number) => {
    ...
    const key = ['browseProducers', JSON.stringify(prefetchParams)];
    mutate(key, vndbStatsApi.browseProducers(prefetchParams), { revalidate: false });
  ...
  const columns: EntityColumn<BrowseProducerItem>[] = [
    {
      key: 'name',
      label: 'Name',
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="flex gap-2">
          ...
          <SimpleSelect
            ...
            value={params.type || ''}
            onChange={(v) => updateParams({ type: v || undefined })}
          />
          <SimpleSelect
      ...
      <div ref={resultsRef} className="scroll-mt-20 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <SimpleSelect
            ...
            onChange={(v) => updateParams({ sort: v as BrowseProducerParams['sort'] })}
            compact
          />
          <button
      ...
      {viewMode === 'table' ? (
        <EntityTable
          ...
          columns={columns}
          getKey={(item) => item.id}
          getLink={(item) => `/stats/producer/${item.id}`}
        ...
        >
          {items.map((item) => {
            ...
            return (
              <EntityCard
                key={item.id}
                link={`/stats/producer/${item.id}`}
                title={displayName}
                subtitle={altName && altName !== displayName ? altName : undefined}
                fields={[
                  { label: 'Type', value: item.type ? (TYPE_LABELS[item.type] || item.type) : '—' },
