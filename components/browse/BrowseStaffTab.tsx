...
import { vndbStatsApi, BrowseStaffItem, BrowseStaffParams } from '@/lib/vndb-stats-api';
import { useBrowseStaff } from '@/lib/vndb-stats-cached';
...
import { EntityTable, EntityColumn, NameCell, CountCell, RoleBadges } from './EntityTable';
...
interface BrowseStaffTabProps {
  isActive?: boolean;
}

export function BrowseStaffTab({ isActive = true }: BrowseStaffTabProps) {
  const { preference } = useTitlePreference();
  const [params, setParams] = useState<BrowseStaffParams>({
    sort: 'vn_count',
    sort_order: 'desc',
    page: 1,
  ...
  const updateParams = (updates: Partial<BrowseStaffParams>) => {
  ...
  const handlePrefetchPage = useCallback((targetPage: number) => {
    ...
    const key = ['browseStaff', JSON.stringify(prefetchParams)];
    mutate(key, vndbStatsApi.browseStaff(prefetchParams), { revalidate: false });
  ...
  const columns: EntityColumn<BrowseStaffItem>[] = [
    {
      key: 'name',
      label: 'Name',
    ...
    },
    {
      key: 'roles',
      label: 'Roles',
      render: (item) => <RoleBadges roles={item.roles} />,
    },
    {
      key: 'description',
      label: 'Description',
      ...
      <div ref={resultsRef} className="scroll-mt-20 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <SimpleSelect
            ...
            onChange={(v) => updateParams({ sort: v as BrowseStaffParams['sort'] })}
            compact
          />
          <button
