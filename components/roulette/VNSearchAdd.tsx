...
import type { VNSearchResult } from '@/lib/vndb-stats-api';
...
interface VNSearchAddProps {
  onAdd: (entry: WheelEntry) => void;
...
export function VNSearchAdd({ onAdd, isItemInList, isAtCapacity, disabled, placeholder, addedLabel, errorMessage }: VNSearchAddProps) {
  ...
  const search = useCallback(async (q: string) => {
    ...
    try {
      // Support VNDB ID lookup (e.g. "v123" or "123")
      ...
      let searchResults = searchRes.results;
  ...
  const handleSelect = useCallback((result: VNSearchResult) => {
    ...
    <div className="relative">
      <div className="relative">
        ...
        <input
          ...
          placeholder={placeholder || (isAtCapacity ? 'Wheel is full (15 max)' : 'Search VNs by name or ID...')}
          disabled={isDisabled}
          role="combobox"
          aria-expanded={isOpen && results.length > 0}
          aria-autocomplete="list"
          aria-controls={isOpen ? 'roulette-search-listbox' : undefined}
          aria-activedescendant={selectedIndex >= 0 ? `roulette-search-option-${selectedIndex}` : undefined}
          aria-label="Search visual novels"
          className="w-full pl-8 pr-7 py-2 text-sm rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-400 focus:ring-2 focus:ring-violet-500 focus:border-violet-500 disabled:opacity-50"
        />
        {isLoading && (
        ...
        >
          {results.map((result, i) => {
            ...
            return (
              <button
                key={result.id}
                id={`roulette-search-option-${i}`}
                role="option"
                aria-selected={i === selectedIndex}
                onClick={() => !alreadyAdded && handleSelect(result)}
                disabled={alreadyAdded}
                className={`w-full flex items-center gap-2.5 px-3 py-2 text-left text-sm transition-colors ${
                  i === selectedIndex ? 'bg-violet-50 dark:bg-violet-900/30' : ''
              ...
              >
                ...
                {alreadyAdded ? (
                  <span className="text-xs text-gray-400 shrink-0">{addedLabel || 'Added'}</span>
                ) : (
      ...
      {isError && !isLoading && results.length === 0 && query.length >= 2 && (
        <div className="absolute z-50 mt-1 w-full rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-lg px-3 py-2">
          <p className="text-sm text-red-400">{errorMessage || 'Search failed. Try again.'}</p>
        </div>
      )}
    </div>
  );
